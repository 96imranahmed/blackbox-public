...
def pro_confused(state_info):
    ...
    state_info.bot.send_text_message(state_info.user.get_user(),'I am confused. Returning to the start')
    return 'start_no_name'

def con_confused(state_info):
    if not state_info.flag_empty:
        state_info.bot.send_text_message(state_info.user.get_user(), "Hey, didn't catch that - try it again")
